...
class BacktestEngine:
    ...
    def _run_window(
        ...
        funding_lookup, funding_miss = self._build_funding_lookup(symbols, window)
        ...
        for i in range(len(master_ts) - 1):
            ...
            # ---- 构造 ctx:snapshot/recent_bars 截至当前bar为止 ----
            snapshot: dict[str, dict] = {}
            recent_bars: dict[str, pd.DataFrame] = {}
            for sym in symbols:
    ...
    def _build_funding_lookup(
        ...
        for sym in symbols:
            ...
            funding_by_symbol[sym] = dict(zip(fdf["timestamp"].astype("int64"), fdf["funding_rate"].astype(float)))
