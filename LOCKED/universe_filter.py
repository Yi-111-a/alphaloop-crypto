...
- ccxt 统一字段 ticker["quoteVolume"] 在 OKX 的 USDT 永续 ticker 上恒为
  ...
  筛选规则的语义没有变。
- fetch_candidates() 原来对 load_markets()/fetch_tickers() 没有任何重试
  逻辑,真实点火时被一次瞬时 SSL 连接中断直接打崩整个 COLD_START 进程
  (ccxt.NetworkError,重跑一次立刻又能通)。新增
  `_with_retry()`(与 data_pipeline.py 的同名方法同一套指数退避重试策略:
  最多3次,失败则向上抛出最后一次异常),包住这两个网络调用——这正是最初
  授权范围里明确写的"重试/限频逻辑",不是新的业务规则。
...
class UniverseFilter:
    ...
    def _with_retry(self, fn: Callable[[], T], description: str) -> T:
        """指数退避重试,与 data_pipeline.py 的同名方法同一套策略:最多
        self.max_retries 次尝试,全部失败则抛出最后一次异常。真实点火时
        瞬时的 SSL/连接中断很常见,不重试会让整个 COLD_START 因为一次可能
        几秒后就自愈的网络抖动而彻底失败。"""
        ...
        for attempt in range(1, self.max_retries + 1):
            try:
                ...
                last_exc = exc
                logger.warning("%s failed (attempt %d/%d): %r", description, attempt, self.max_retries, exc)
    ...
    def fetch_candidates(self) -> list[dict]:
        ...
        """
        markets = self._with_retry(self.exchange.load_markets, "load_markets")
        tickers = self._with_retry(self.exchange.fetch_tickers, "fetch_tickers")
