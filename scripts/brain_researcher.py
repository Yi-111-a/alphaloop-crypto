...
def build_review_prompt(
    *,
    ...
    return (
        ...
        "- ctx.positions: dict[str, PerpPosition]。PerpPosition的字段**只有**这6个"
        "(2026-07-19教训:有研究员猜了不存在的net_size字段,整轮提案直接报废):"
        "symbol: str / side: 'long'或'short' / notional: float(名义USDT) / "
        "entry_price: float / margin: float / leverage: int。没有别的字段,不要猜。\n"
        ...
        "**K线周期是1小时**(2026-07-19教训:有研究员按5分钟bar设计回看窗口,"
        "所有时间尺度实际放大了12倍还不自知)——12根bar=12小时,不是1小时;"
        "回测和实盘都是同一个1小时周期,你的执行循环虽然每5分钟跑一次,但"
        "K线数据每小时才多一根,5分钟层面变化的只有snapshot里的最新价。\n"
