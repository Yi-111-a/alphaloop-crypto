...
运行前置条件(缺一不可,缺了本脚本会在启动时就明确报错退出,不会静默降级):
    - 环境变量 ANTHROPIC_API_KEY 已设置(Trader/Researcher/Reflector 的
      llm_client 需要真实 LLM 推理能力,这是 M5 阶段三启动前必须解决的
      前置阻塞项,详见与用户的对话记录)。
    - 已安装 anthropic SDK(requirements.txt 未包含,需要单独
...
def build_llm_client():
    """构造真实 Claude API 调用。缺 API key / SDK 时明确报错退出,不降级为
    假数据——一个"看起来在跑但其实是假LLM输出"的点火比不点火更危险。"""
    api_key = os.environ.get("ANTHROPIC_API_KEY")
    if not api_key:
        raise SystemExit(
            "ANTHROPIC_API_KEY 未设置。Trader/Researcher/Reflector 需要真实 LLM 推理"
            "能力才能产出有意义的输出(不是可以用假数据代替的连通性问题)——"
            "这是 M5 阶段三启动前的前置阻塞项,已经在对话里向用户报告。"
        )
    try:
        import anthropic
    except ImportError as exc:
        raise SystemExit(
            "anthropic SDK 未安装。`pip install anthropic` 后重试。"
        ) from exc
